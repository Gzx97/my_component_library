...
/// 文本控件
/// 设计原则：
/// 1.为了使用更方便，所以对系统组件进行的扩展，需兼容系统控件所有功能，不能让用户使用Oeesign时，因不能满足系统功能而弃用。
/// 2.非系统已有属性，尽量添加注释
///
...
/// 替换：$1$2: this.$1$2,
///
class OeText extends StatelessWidget {
  const OeText(
    this.data, {
    this.font,
    this.fontWeight,
  ...
  Widget build(BuildContext context) {
    ...
    if (forceVerticalCenter && kTextForceVerticalCenterEnable) {
      ...
      paddingConfig ??= OeTextPaddingConfig.getDefaultConfig();
  ...
  /// 提取成方法，允许业务定义自己的OeTextConfiguration
  OeTextConfiguration? getConfiguration(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<OeTextConfiguration>();
  }
...
/// TextSpan的Oeesign扩展，将部分TextStyle中的参数扁平化。
class OeTextSpan extends TextSpan {
  /// 构造参数，扩展参数释义可参考[OeText]中字段注释
  OeTextSpan({
    BuildContext?
...
/// 存储可以自定义OeText居中算法数据的内部控件
class OeTextConfiguration extends InheritedWidget {
  /// forceVerticalCenter=true时，内置padding配置
  final OeTextPaddingConfig? paddingConfig;

  /// 全局字体,kTextNeedGlobalFontFamily=true时生效
  final FontFamily? globalFontFamily;

  const OeTextConfiguration(
      {Key? key,
  ...
  @override
  bool updateShouldNotify(covariant OeTextConfiguration oldWidget) {
    return paddingConfig != oldWidget.paddingConfig;
  }
}

/// 通过Padding自定义OeText居中算法
class OeTextPaddingConfig {
  static OeTextPaddingConfig? _defaultConfig;
  static final Map _cacheMap = {};

  /// 获取默认配置
  static OeTextPaddingConfig getDefaultConfig() {
    _defaultConfig ??= OeTextPaddingConfig();
