...
class OeSlidePopupRoute<T> extends PopupRoute<T> {
  OeSlidePopupRoute({
        ...
        if (!modalBarrierFull)
          _getPositionWidget(
            ...
            IgnorePointer(
              ...
              child: Container(
                color: _barrierColor
                    .withAlpha((animValue * _barrierColor.alpha).toInt()),
  ...
  Widget _getPositionWidget(BuildContext context, Widget child) {
    ...
    var _modalHeight = (modalHeight ?? screenSize.height)
        .clamp(0, screenSize.height - _modalTop)
        .toDouble();
    var _modalWidth = (modalWidth ?? screenSize.width)
        .clamp(0, screenSize.width - _modalLeft)
        .toDouble();
    return Positioned(
      top: _modalTop,
    ...
    switch (slideTransitionFrom) {
      case SlideTransitionFrom.top:
        return Rect.fromLTWH(
            0, size.height * (1 - animValue), size.width, size.height);
      case SlideTransitionFrom.right:
        return Rect.fromLTWH(0, 0, size.width * animValue, size.height);
      case SlideTransitionFrom.left:
        return Rect.fromLTWH(
            size.width * (1 - animValue), 0, size.width, size.height);
      case SlideTransitionFrom.bottom:
