...
/// 控制CheckBoxGroup
///
class OeCheckboxGroupController {
  OeCheckboxGroupState? _state;
...
///
/// child的属性可以是任意包含OeCheckBox的容器组件，例如：
/// ```dart
/// OeCheckboxGroup(
///   child: Row(
///     children: [
...
///
///
class OeCheckboxGroup extends StatefulWidget {
  const OeCheckboxGroup(
      {required this.child,
      Key? key,
      this.onChangeGroup,
      this.controller,
  ...
  /// 可以通过控制器操作勾选状态
  final OeCheckboxGroupController? controller;
  ...
  /// CheckBox复选框样式：圆形或方形
  final OeCheckboxStyle? style;

  /// 文字相对icon的方位
  final OeContentDirection? contentDirection;
  ...
  State<StatefulWidget> createState() {
    return OeCheckboxGroupState();
  }
}

class OeCheckboxGroupState extends State<OeCheckboxGroup> {
  ///
  /// 管理所有子CheckBox的状态
  ///
  ...
  @override
  void didUpdateWidget(OeCheckboxGroup oldWidget) {
  ...
  Widget build(BuildContext context) {
    return OeCheckboxGroupInherited(this, widget.child);
  }
}

class OeCheckboxGroupInherited extends InheritedWidget {
  final OeCheckboxGroupState state;

  ///
  /// 获取树上的Group节点
  ///
  static OeCheckboxGroupInherited? of(BuildContext context) {
    return context
        .dependOnInheritedWidgetOfExactType<OeCheckboxGroupInherited>();
  }

  const OeCheckboxGroupInherited(this.state, Widget child, {Key? key})
      : super(child: child, key: key);

  @override
  bool updateShouldNotify(covariant OeCheckboxGroupInherited oldWidget) {
    return true;
  }
}

class OeCheckboxGroupContainer extends OeCheckboxGroup {
  OeCheckboxGroupContainer({
    Key? key,
    Widget? child, // 使用child 则请勿设置direction
    Axis? direction, // direction 对 directionalTdRadios 起作用
    List<OeCheckbox>? directionalTdCheckboxes,
    ...
    OeCheckboxStyle? style, // 勾选样式
    OeCheckboxGroupController? controller,
    ...
    VoidCallback? onOverloadChecked,
          ...
          if (direction != null && directionalTdCheckboxes == null) {
            throw FlutterError(
                '[OeCheckboxGroupContainer] direction and directionalTdCheckboxes must set at the same time');
          }
          // 未使用direction则必须设置child
          if (direction == null && child == null) {
            throw FlutterError(
                '[OeCheckboxGroupContainer] direction means use child as the exact one, but child is null');
          }
          // 横向单选框 每个选项有字数限制
          if (direction == Axis.horizontal && directionalTdCheckboxes != null) {
            ...
            var maxWordCount = 2;
            var tips =
                '[OeCheckboxGroupContainer] checkbox title please not exceed $maxWordCount words.\n'
          ...
          if (cardMode == true) {
            ...
            directionalTdCheckboxes!.forEach((element) {
              ...
              if (element.cardMode == false) {
                throw FlutterError(
                    'if use cardMode at OeCheckboxGroupContainer, then every '
                    'OeCheckbox should set it\'s own carMode to true.');
              }
  ...
  @override
  State<StatefulWidget> createState() {
    return OeCheckboxGroupContainerState();
  }
}

class OeCheckboxGroupContainerState extends OeCheckboxGroupState {}
