...
import 'oe_rate_overlay.dart';
import 'oe_rate_tips.dart';
...
/// 评分组件
class OeRate extends StatefulWidget {
  const OeRate({
    super.key,
  ...
  /// 评分图标的颜色，示例：[选中颜色] / [选中颜色，未选中颜色]，默认：[OeTheme.of(context).warningColor5, OeTheme.of(context).grayColor4]
  ...
  /// 评分图标的间距，默认：OeTheme.of(context).spacer8
  final double? gap;

  /// 自定义评分图标，[选中和未选中图标] / [选中图标，未选中图标]，默认：[OeIcons.star_filled]
  ...
  /// 评分图标与辅助文字的间距，默认：[OeTheme.of(context).spacer16]
  final double? iconTextGap;

  @override
  _OeRateState createState() => _OeRateState();
}

class _OeRateState extends State<OeRate> with TickerProviderStateMixin {
  /// 节流
  ...
  void initState() {
    ...
    _overlay =
        OeRateOverlay(context: context, builder: (context) => _buildOverlay())
          ..show();
  ...
  @override
  void didUpdateWidget(OeRate oldWidget) {
            ...
            children: List.generate(widget.count ?? 5, (index) {
              ...
              return Padding(
                padding: EdgeInsets.only(
                    right:
                        isLast ? 0 : widget.gap ?? OeTheme.of(context).spacer8),
  ...
  Widget _getDefText() {
    ...
    return Padding(
      padding: widget.direction == Axis.horizontal
          ? EdgeInsets.only(
              left: widget.iconTextGap ?? OeTheme.of(context).spacer16)
          : EdgeInsets.only(
              top: widget.iconTextGap ?? OeTheme.of(context).spacer8),
      child: SizedBox(
        ...
        child: OeText(
          ...
          font: notRated
              ...
              : OeTheme.of(context).fontTitleMedium,
          textColor: notRated
  ...
  Color _getIconColor({double? value, bool? isActive}) {
    return (value != null && _activeValue >= value) ||
        ...
        ? widget.color?.getOrNull(0) ?? OeTheme.of(context).warningColor5
        : widget.color?.getOrNull(1) ?? OeTheme.of(context).grayColor4;
  }

  IconData _getIcon({double? value, bool? isActive}) {
    final selectIcon = widget.icon?.getOrNull(0) ?? OeIcons.star_filled;
  ...
  Widget _buildOverlay() {
    ...
    return Positioned(
      top: widget.placement == PlacementEnum.top
          ? rateOffset.dy - OeTheme.of(context).spacer8 - _tipSize.height
          : rateOffset.dy + OeTheme.of(context).spacer8 + rateSize.height,
