...
/// 下拉菜单构建器
typedef OeDropdownItemBuilder = List<OeDropdownItem> Function(
...
/// 下拉菜单
class OeDropdownMenu extends StatefulWidget {
  const OeDropdownMenu({
    Key? key,
    this.builder,
    this.items,
    this.closeOnClickOverlay = true,
    this.direction = OeDropdownMenuDirection.auto,
    this.duration = 200.0,
  ...
  /// 下拉菜单构建器，优先级高于[items]
  final OeDropdownItemBuilder? builder;

  /// 下拉菜单
  final List<OeDropdownItem>? items;

  /// 是否在点击遮罩层后关闭菜单
  final bool? closeOnClickOverlay;

  /// 菜单展开方向（down、up、auto）
  final OeDropdownMenuDirection? direction;
  ...
  /// [OeDropdownItem.label]和[arrowIcon]/[OeDropdownItem.arrowIcon]的对齐方式
  ...
  @override
  _OeDropdownMenuState createState() => _OeDropdownMenuState();
}

class _OeDropdownMenuState extends State<OeDropdownMenu>
    with TickerProviderStateMixin {
  List<OeDropdownItem>? _items;
  List<AnimationController>? _iconControllers;
  late List<Animation<double>> _iconAnimations;
  late List<bool> _isOpened;
  OeDropdownPopup? _dropdownPopup;
  ...
  @override
  void didUpdateWidget(OeDropdownMenu oldWidget) {
  ...
  Widget _getIcon(int index) {
    var arrowIcon = _items![index].arrowIcon ??
        widget.arrowIcon ??
        (widget.direction == OeDropdownMenuDirection.up
            ? OeIcons.caret_up_small
            : OeIcons.caret_down_small);
  ...
  void _openMenu(int index) async {
    ...
    _dropdownPopup ??= OeDropdownPopup(
      child: _items![index],
