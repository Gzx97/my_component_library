...
/// 计时组件
class OeTimeCounter extends StatefulWidget {
  const OeTimeCounter({
    Key? key,
    this.autoStart = true,
    ...
    this.size = OeTimeCounterSize.medium,
    this.splitWithUnit = false,
    this.theme = OeTimeCounterTheme.defaultTheme,
    required this.time,
    this.style,
    this.onChange,
    this.onFinish,
    this.direction = OeTimeCounterDirection.down,
    this.controller,
  }) : super(key: key);
  ...
  /// 尺寸
  final OeTimeCounterSize size;

  /// 使用时间单位分割
  final bool splitWithUnit;

  /// 风格
  final OeTimeCounterTheme theme;

  /// 必需；计时时长，单位毫秒
  final int time;

  /// 自定义样式，有则优先用它，没有则根据size和theme选取
  final OeTimeCounterStyle? style;
  ...
  /// 计时方向，默认倒计时
  final OeTimeCounterDirection direction;

  /// 控制器，可控制开始/暂停/继续/重置
  final OeTimeCounterController? controller;

  @override
  _OeTimeCounterState createState() => _OeTimeCounterState();
}

class _OeTimeCounterState extends State<OeTimeCounter>
    with SingleTickerProviderStateMixin {
  late OeTimeCounterStyle _style;
  ...
  @override
  void didUpdateWidget(OeTimeCounter oldWidget) {
  ...
  void startTimer() {
    ...
    _ticker ??= createTicker((Duration elapsed) {
      if ((widget.direction == OeTimeCounterDirection.down && _time > 0) ||
          widget.direction == OeTimeCounterDirection.up && _time < _maxTime) {
        setState(() {
          if (widget.direction == OeTimeCounterDirection.down) {
            _time =
  ...
  void _onControllerChanged() {
    switch (widget.controller?.value) {
      case OeTimeCounterStatus.start:
        startTimer();
        break;
      case OeTimeCounterStatus.pause:
        pauseTimer();
        break;
      case OeTimeCounterStatus.resume:
        resumeTimer();
        break;
      case OeTimeCounterStatus.reset:
