...
/// 分割线
/// 对于非flutter原有的控件，则只需满足Oeesign规范即可；
/// 如果有业务在实际使用，还需兼容实际业务场景。
class OeDivider extends StatelessWidget {
  const OeDivider({
    Key? key,
    this.color,
    this.margin,
          ...
          child: Row(
            ...
            children: [
              _buildLine(
                ...
                width: 16,
                height: height ?? 0.5,
                color: color ?? OeTheme.of(context).grayColor3,
              ),
              Padding(
                  padding:
                      gapPadding ?? const EdgeInsets.only(left: 8, right: 8),
                  child: _buildMiddleWidget(context)),
              Expanded(
                  child: Center(
                      child: _buildLine(
