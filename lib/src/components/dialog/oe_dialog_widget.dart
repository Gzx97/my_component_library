...
/// OeDialog手脚架
class OeDialogScaffold extends StatelessWidget {
  const OeDialogScaffold({
    Key? key,
    required this.body,
...
/// 弹窗标题
class OeDialogTitle extends StatelessWidget {
  const OeDialogTitle({
    Key? key,
    this.title,
...
/// 弹窗内容
class OeDialogContent extends StatelessWidget {
  const OeDialogContent({
    Key? key,
    this.content,
...
/// 弹窗信息
class OeDialogInfoWidget extends StatelessWidget {
  const OeDialogInfoWidget({
    Key? key,
    this.title,
    this.titleColor = Colors.black,
    this.titleAlignment,
          ...
          children: [
            Expanded(
              child: OeDialogButton(
                ...
                buttonTextSize: leftBtn.titleSize,
                buttonStyle: leftBtn.style,
                buttonType: leftBtn.type ?? OeButtonType.text,
                buttonTheme: leftBtn.theme,
                height: leftBtn.height,
            ...
            Expanded(
              child: OeDialogButton(
                ...
                buttonStyle: leftBtn.style,
                buttonType: leftBtn.type ?? OeButtonType.text,
                buttonTheme: leftBtn.theme ?? OeButtonTheme.primary,
                height: rightBtn.height,
                buttonTextFontWeight: rightBtn.fontWeight ?? FontWeight.w600,
                onPressed: () {
                  if (rightBtn.action != null) {
...
/// 弹窗标题
class OeDialogButton extends StatelessWidget {
  const OeDialogButton({
    Key? key,
    this.buttonText,
  ...
  /// 按钮样式
  final OeButtonStyle? buttonStyle;

  /// 按钮类型
  final OeButtonType? buttonType;

  /// 按钮主题
  final OeButtonTheme? buttonTheme;
  ...
  Widget build(BuildContext context) {
    return OeButton(
      onTap: onPressed,
      style: buttonStyle,
      type: buttonType ?? OeButtonType.fill,
      theme: buttonTheme,
      text: buttonText,
      textStyle: TextStyle(
