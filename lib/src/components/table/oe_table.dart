...
enum OeTableColFixed { left, right, none }

enum OeTableColAlign { left, center, right }

typedef OnCellTap = void Function(int rowIndex, dynamic row, OeTableCol col);
...
class OeTableCol {
  OeTableCol({
    ...
    this.colKey,
    this.width,
    this.fixed = OeTableColFixed.none,
    this.ellipsis,
    this.ellipsisTitle,
    this.cellBuilder,
    this.align = OeTableColAlign.left,
    this.sortable = false,
  });
...
class OeTable extends StatefulWidget {
  const OeTable({
    super.key,
    this.bordered,
  ...
  /// 列配置
  final List<OeTableCol> columns;

  /// 数据源
  final List<dynamic>? data;

  /// 空表格呈现样式
  final OeTableEmpty? empty;
  ...
  @override
  State<OeTable> createState() => OeTableState();
}

class OeTableState extends State<OeTable> {
  bool? _sortable;
  String? _sortKey;
  final _scrollController = ScrollController();

  /// 获取单元格对齐方式
  Alignment _getVerticalAlign(OeTableColAlign x) {
    var xPos = 0.0;
    switch (x) {
      case OeTableColAlign.left:
        xPos = -1;
        break;
      case OeTableColAlign.center:
        xPos = 0;
        break;
      case OeTableColAlign.right:
        xPos = 1;
        break;
    }
  ...
  /// 过滤列配置
  List<OeTableCol> _getCol(OeTableColFixed fixed) {
    return widget.columns.where((col) => col.fixed == fixed).toList();
  }

  /// 生成表头
  Widget _getTableHeader(BuildContext context) {
    var fixedLeftCol = _getCol(OeTableColFixed.left);
    var fixedNonCol = _getCol(OeTableColFixed.none);
    var fixedRightCol = _getCol(OeTableColFixed.right);
    var start = 0;
      ...
      return Align(
        ...
        child: Padding(
          ...
          child: widget.loadingWidget ??
              const OeLoading(size: OeLoadingSize.large),
        ),
      );
    }
  ...
  /// 获取单元格
  Widget _getCell(OeTableCol col, bool isHeader, dynamic data, int index,
      bool fixedBorder) {
    ...
    if (fixedBorder && col.fixed == OeTableColFixed.left) {
      rightBorder = doubleBorder;
    }
    if (fixedBorder && col.fixed == OeTableColFixed.right) {
  ...
  /// 获取单元格内容
  Widget _getCellText(OeTableCol col, String title, bool ellipsis,
      bool isHeader, bool sortable, int index) {
    var overflow = ellipsis ? TextOverflow.ellipsis : TextOverflow.visible;
    var titleWidget = OeText(title,
        maxLines: 1,
        overflow: overflow,
        style: TextStyle(
          color: isHeader
              ? OeTheme.of(context).fontGyColor3
    ...
    // 表头（需考虑排序模式）
    if (isHeader) {
      var selectColor = OeTheme.of(context).brandNormalColor;
      var unSelectColor = OeTheme.of(context).fontGyColor3;
      return Row(
  ...
  Widget _getFixedTable(BuildContext context) {
    ...
    for (var tableCol in widget.columns) {
      if (tableCol.fixed == OeTableColFixed.left ||
          tableCol.fixed == OeTableColFixed.right) {
    ...
    if ((width - fixedCellsWidth) < fixedNonCellsWidth) {
      ...
      return Container(
        width: width,
        color: widget.backgroundColor ?? OeTheme.of(context).whiteColor1,
        child: Row(
      ...
      child: Align(
        ...
        child: Padding(
          ...
          child: OeEmpty(
            image: Visibility(
              visible: false,
              child: OeImage(assetUrl: widget.empty?.assetUrl ?? ''),
            ),
  ...
  List<Widget> _getVerticalCell(
      List<OeTableCol> cols, List<List<String>> titles, double cellWidth) {
    var rows = <Widget>[];
  ...
  /// 获取每列单元格内容
  List<List<String>> _getCellsText(List<OeTableCol> cols) {
  ...
  Widget build(BuildContext context) {
    ...
    var width = widget.width ?? MediaQuery.of(context).size.width;
    var fixedCols = [
      ..._getCol(OeTableColFixed.left),
      ..._getCol(OeTableColFixed.right)
    ];
