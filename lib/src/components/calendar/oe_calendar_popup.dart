...
class OeCalendarPopup {
  OeCalendarPopup(
    ...
    if (builder == null && child == null) {
      throw FlutterError('[OeCalendarPopup] builder or child must be not null');
    }
    if (visible == true) {
      show();
    }
  ...
  /// 日历控件
  final OeCalendar? child;

  /// 点击确认按钮时触发
  final void Function(List<int> value)? onConfirm;

  static OeSlidePopupRoute? _calendarPopup;
  ...
  void show() {
    ...
    _calendarPopup = OeSlidePopupRoute(
      isDismissible: false,
...
class OeCalendarInherited extends InheritedWidget {
  const OeCalendarInherited({
  ...
  @override
  bool updateShouldNotify(covariant OeCalendarInherited oldWidget) {
    return false;
  }

  static OeCalendarInherited? of(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<OeCalendarInherited>();
  }
}
