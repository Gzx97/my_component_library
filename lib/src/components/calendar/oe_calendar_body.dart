...
class OeCalendarBody extends StatelessWidget {
  const OeCalendarBody({
    Key? key,
    this.maxDate,
    this.minDate,
    required this.type,
  ...
  final Widget Function(Oeate? date, List<Oeate?> dateList,
      Map<DateTime, List<Oeate?>> data, int rowIndex, int colIndex) builder;
  ...
  Widget build(BuildContext context) {
    ...
    final data = <DateTime, List<Oeate?>>{};
          ...
          children: [
            SizedBox(
              ...
              child: monthTitleBuilder?.call(context, monthDate) ??
                  OeText(monthDateText, style: monthTitleStyle),
            ),
            ...List.generate(
  ...
  List<Oeate?> _getDaysInMonth(DateTime curDate, DateTime min, DateTime max) {
    final daysInMonth =
        List<Oeate?>.generate(_getPreOffset(curDate), (index) => null);
