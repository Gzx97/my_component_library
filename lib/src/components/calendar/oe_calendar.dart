...
export 'oe_calendar_body.dart';
export 'oe_calendar_cell.dart';
export 'oe_calendar_header.dart';
export 'oe_calendar_popup.dart';
export 'oe_calendar_style.dart';

typedef CalendarFormat = Oeate? Function(Oeate? day);
...
/// 日历组件
class OeCalendar extends StatefulWidget {
  const OeCalendar({
    Key? key,
  ...
  /// 自定义样式
  final OeCalendarStyle? style;

  /// 选中值变化时触发
  final void Function(List<int> value)? onChange;

  /// 点击日期时触发
  final void Function(int value, DateSelectType type, Oeate tdate)? onCellClick;

  /// 长安日期时触发
  final void Function(int value, DateSelectType type, Oeate tdate)?
      onCellLongPress;
  ...
  @override
  _OeCalendarState createState() => _OeCalendarState();
}

class _OeCalendarState extends State<OeCalendar> {
  late List<String> weekdayNames;
  late List<String> monthNames;
  late OeCalendarInherited? inherited;
  late OeCalendarStyle _style;
  ...
  void didChangeDependencies() {
    ...
    _style = widget.style ?? OeCalendarStyle.generateStyle(context);
  }

  @override
  void didUpdateWidget(OeCalendar oldWidget) {
  ...
  Widget build(BuildContext context) {
    inherited = OeCalendarInherited.of(context);
    _initValue();
    timePickerModelList.clear();
    final verticalGap = _style.verticalGap ?? OeTheme.of(context).spacer8;
    return Container(
      ...
      child: Column(
        children: [
          OeCalendarHeader(
            ...
            weekdayGap: OeTheme.of(context).spacer4,
            padding: OeTheme.of(context).spacer16,
          ...
          Expanded(
            child: OeCalendarBody(
              type: widget.type ?? CalendarType.single,
              firstDayOfWeek: widget.firstDayOfWeek ?? 0,
              maxDate: widget.maxDate,
              minDate: widget.minDate,
              value: widget._value,
              bodyPadding: _style.bodyPadding ?? OeTheme.of(context).spacer16,
            ...
            inherited?.confirmBtn ??
                Padding(
                  padding: EdgeInsets.symmetric(
                      vertical: OeTheme.of(context).spacer16),
                  child: OeButton(
